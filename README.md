...
```mermaid
graph TB
    subgraph UI["用户界面层 screens/components/ink"]
        REPL["REPL.tsx<br/>主交互界面"]
        Prompt["PromptInput<br/>文本输入(Vim/补全)"]
        MsgList["Messages<br/>消息列表(虚拟滚动)"]
        PermDlg["权限对话框<br/>工具审批"]
    end

    subgraph Engine["查询引擎层 query/ QueryEngine"]
        QueryLoop["query()<br/>核心对话循环"]
        QESDK["QueryEngine<br/>SDK/headless封装"]
        Micro["Microcompact<br/>轻量压缩"]
        Auto["Autocompact<br/>自动压缩"]
        Reactive["Reactive Compact<br/>错误时压缩"]
        TokenBudget["Token Budget<br/>预算管理"]
        StopHooks["Stop Hooks<br/>后置钩子"]
        Streaming["StreamingToolExecutor<br/>并行工具执行"]
    end

    subgraph Tools["工具系统层 tools/"]
        FileTools["文件工具<br/>Read/Edit/Write/Glob/Grep"]
        ShellTools["Shell工具<br/>Bash/PowerShell"]
        AgentTools["Agent工具<br/>AgentTool/TaskCreate"]
        WebTools["Web工具<br/>WebFetch/WebSearch"]
        SystemTools["系统工具<br/>LSP/MCP/Config"]
        PlanTools["规划工具<br/>EnterPlan/ExitPlan"]
        Worktree["Worktree工具<br/>EnterWT/ExitWT"]
        TaskTools["任务工具<br/>TaskCreate/Stop/List"]
        MCPDynamic["MCP动态工具<br/>运行时注册"]
    end
    ...
    subgraph Cross["横切系统"]
        Swarm["Swarm多Agent<br/>utils/swarm/<br/>Tmux/iTerm/InProcess"]
        Coordinator["Coordinator<br/>coordinator/"]
        Bridge["Bridge远程控制<br/>bridge/ 31文件"]
        MemorySys["记忆系统<br/>memdir/ MEMORY.md"]
        Skills["技能系统<br/>skills/ 12+内置"]
        Plugins["插件系统<br/>plugins/"]
    end

    subgraph State["状态与存储层 state/"]
        AppState["AppState<br/>全局状态 store"]
        Bootstrap["bootstrap/state.ts<br/>可变会话状态"]
        CostTrack["cost-tracker<br/>费用追踪"]
        Settings["Settings<br/>用户配置"]
        History["Session History<br/>会话历史"]
    end
    ...
    QueryLoop -->|"流式API"| API
    QueryLoop -->|"tool_use"| Streaming
    Streaming --> FileTools
    Streaming --> ShellTools
    Streaming --> AgentTools
    Streaming --> WebTools
    Streaming --> SystemTools
    Streaming --> PlanTools
    Streaming --> Worktree
    Streaming --> TaskTools
    Streaming --> MCPDynamic
    QueryLoop -->|"权限检查"| PermDlg
    ...
    QueryLoop -.->|"多Agent"| Swarm
    QueryLoop -.->|"协调模式"| Coordinator
    Bridge -.->|"远程会话"| QueryLoop
    Skills -.-> QueryLoop
    Plugins -.-> QueryLoop
    ...
    style State fill:#e0f2f1,stroke:#333
```
...
sequenceDiagram
    ...
    alt 斜杠命令
        ...
        loop 对话循环
            ...
            alt 提示过长
                Query->>Query: ReactiveCompact重试
            else 输出Token超限
                Query->>Query: 升级到64k输出重试
            else 模型不可用
                Query->>Query: 模型回退重试
            end
        end
...
```mermaid
graph LR
    User["用户"] -->|"输入"| Input["PromptInput<br/>文本输入"]
    Input -->|"提交"| Process["processUserInput()<br/>输入路由"]

    Process -->|"普通对话"| Query["query()<br/>核心循环"]
    Process -->|"斜杠命令"| CmdExec["命令执行器<br/>100+命令"]
    Process -->|"Bash命令"| BashExec["Bash执行器"]

    Query -->|"压缩"| Compact["Auto/Micro/Reactive<br/>上下文压缩"]
    Query -->|"API调用"| API["services/api/<br/>流式客户端"]
    API -->|"流式响应"| Query

    Query -->|"tool_use"| ToolExec["工具执行<br/>43+内置 + MCP动态"]
    ToolExec -->|"结果"| Query
    ToolExec -->|"文件变更"| FS["文件系统"]
    ToolExec -->|"状态更新"| AppState["AppState<br/>全局状态"]

    Query -->|"记忆预取"| MemLoad["memdir/<br/>MEMORY.md加载"]
    Query -->|"后处理"| StopHook["StopHooks<br/>记忆提取/梦境"]

    Query -->|"多Agent"| Swarm["Swarm系统<br/>Teammate/Coordinator"]
    Query -->|"渲染"| UI["REPL UI<br/>终端显示"]

    AppState -->|"持久化"| Settings["配置文件"]
    AppState -->|"历史"| History["会话历史"]
```
