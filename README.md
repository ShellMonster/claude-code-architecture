...
graph TB
    subgraph UI["用户界面层"]
        ...
        PromptInput["PromptInput<br/>文本输入(Vim/补全)"]
        Messages["Messages<br/>消息列表(虚拟滚动)"]
    ...
    subgraph Engine["查询引擎层"]
        ...
        QueryEngine["QueryEngine<br/>SDK封装"]
    ...
    subgraph Tools["工具系统层"]
        ...
        AgentTools["Agent工具<br/>TaskCreate/AgentTool"]
    ...
    subgraph Services["服务层"]
        ...
        MCP["services/mcp/<br/>MCP连接管理"]
        LSP["services/lsp/<br/>语言服务器协议"]
        Analytics["services/analytics/<br/>遥测分析"]
        SessionMemory["services/SessionMemory/<br/>会话记忆提取"]
    end

    subgraph State["状态与存储层"]
        AppState["AppState<br/>全局状态(Zustand-like)"]
        Memory["Memory System<br/>MEMORY.md持久化记忆"]
        Settings["Settings<br/>用户配置"]
        History["Session History<br/>会话历史"]
    end

    REPL --> QueryLoop
    QueryLoop -->|"tool_use"| FileTools
    QueryLoop -->|"tool_use"| ShellTools
    QueryLoop -->|"tool_use"| AgentTools
    QueryLoop -->|"tool_use"| WebTools
    QueryLoop -->|"tool_use"| SystemTools
    QueryLoop --> API
    API -->|"流式响应"| QueryLoop
...
graph LR
    ...
    Input -->|"提交"| Process["processUserInput()<br/>命令/文本路由"]

    Process -->|"普通对话"| Query["query()<br/>核心循环"]
    Process -->|"斜杠命令"| CmdExec["命令执行器<br/>60+命令"]

    Query -->|"API调用"| API["services/api/<br/>流式客户端"]
    API -->|"响应"| Query

    Query -->|"tool_use"| ToolExec["工具执行<br/>43+工具"]
    ToolExec -->|"结果"| Query
    ToolExec -->|"文件变更"| FS["文件系统"]
    ToolExec -->|"状态更新"| AppState["AppState<br/>全局状态"]

    Query -->|"压缩"| Compact["Autocompact<br/>上下文压缩"]
    Query -->|"记忆"| Memory["SessionMemory<br/>记忆提取"]
